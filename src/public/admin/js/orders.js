...
const Orders = {
    ...
    render() {
        ...
        if (!filtered.length) {
            tbody.innerHTML = '<tr><td colspan="7" class="py-8 text-center text-white">No orders found</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = filtered.map(order => {
            ...
            const statusColors = {
                ...
                'refunded': 'bg-gray-500/20 text-white'
            };
            const statusClass = statusColors[order.status] || 'bg-gray-500/20 text-white';
            ...
            return `
                <tr class="border-t border-gray-800 hover:bg-gray-800/30">
                    ...
                    <td class="py-4 px-4 text-white">${Utils.formatDate(order.created_at)}</td>
                    <td class="py-4 px-4">
    ...
    async view(id) {
        ...
        if (memberName && memberName !== memberEmail) {
            customerDisplay = `<p class="font-medium">${Utils.escapeHtml(memberName)}</p>
                              <p class="text-sm text-white">${memberEmail}</p>
                              <p class="font-mono text-xs text-white">${walletShort}</p>`;
        } else if (memberEmail) {
        ...
        content.innerHTML = `
            <div class="space-y-6">
                <div class="grid grid-cols-2 gap-4">
                    <div><p class="text-white text-sm">Order Number</p><p class="font-medium">${order.order_number || order.id?.slice(0,8)}</p></div>
                    <div><p class="text-white text-sm">Status</p><span class="px-2 py-1 rounded-full text-xs status-${order.status}">${order.status}</span></div>
                    <div><p class="text-white text-sm">Customer</p>${customerDisplay}</div>
                    <div><p class="text-white text-sm">Date</p><p>${Utils.formatDateTime(order.created_at)}</p></div>
                </div>
                <div>
                    <p class="text-white text-sm mb-2">Items</p>
                    <div class="space-y-2">
                        ${items.map(item => `<div class="flex justify-between p-3 bg-gray-800/50 rounded-lg"><div><p class="font-medium">${item.service_name || item.name || 'Item'}</p><p class="text-sm text-white">Qty: ${item.quantity || 1}</p></div><p class="font-medium">${Utils.formatCurrency(item.total || item.price)}</p></div>`).join('')}
                    </div>
                </div>
                ...
                <div>
                    <p class="text-white text-sm mb-2">Update Status</p>
