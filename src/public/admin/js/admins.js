...
const Admins = {
    ...
    render() {
        ...
        if (!this.data.length) {
            tbody.innerHTML = '<tr><td colspan="5" class="py-8 text-center text-gray-500">No admins found</td></tr>';
            return;
        }
        ...
        tbody.innerHTML = this.data.map(a => `
            <tr class="border-t border-gray-800 hover:bg-gray-800/30">
                <td class="py-4 px-4">
                    ...
                    ${a.name ? `<div class="text-xs text-gray-400">${a.name}</div>` : ''}
                </td>
                <td class="py-4 px-4">
                ...
                <td class="py-4 px-4 text-gray-400">${a.last_login ? Utils.formatDateTime(a.last_login) : 'Never'}</td>
